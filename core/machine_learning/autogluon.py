...
from core.machine_learning.forecast_base_model import ForecastBaseModel
...
class AutoGluonForecaster(ForecastBaseModel):
    ...
    def train_on_thread(self,
                        training_data: pd.DataFrame,
                        target_variable: str,
                        prediction_horizon: int):
        t = threading.Thread(target=self.train_model, args=(training_data,
                                                            target_variable,
                                                            prediction_horizon))
        t.start()
    ...
    def predict_model(self, past_data:pd.DataFrame):
        if not self.trained:
            raise ValueError('Model has to be trained before it can predict')
        past_data[self.item_id_name] = self.item_id
...
if __name__ == '__main__':
    ...
    ### preprocess data
    # load data
    data_path = Path(ROOT_DIR) / 'data' / '01_input' / '02_electric_loadprofiles_HTW' /\
                'el_loadprofiles_HTW_processed_0.csv'
    ...
    pred_1.train_model(train_data_df, 'target', 48)
